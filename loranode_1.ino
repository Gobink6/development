...
static const u1_t DEVEUI[8]  = { 0x2D, 0x50, 0x0F, 0x65, 0x6E, 0xAF, 0xF0, 0x00 };
static const u1_t APPEUI[8] = { 0xDE, 0xED, 0x00, 0xD0, 0x7E, 0xD5, 0xB3, 0x70 };

// This key should be in big endian format (or, since it is not really a
// number but a block of memory, endianness does not really apply). In
// practice, a key taken from ttnctl can be copied as-is.
// The key shown here is the semtech default key.
static const u1_t APPKEY[16] = { 0x21, 0xFD, 0x31, 0x60, 0x47, 0xAC, 0x08, 0x55, 0xFE, 0xB4, 0x3F, 0xC4, 0xBA, 0x7F, 0xB4, 0x46 };
...
void do_send(osjob_t* j) {
  ...
  if (LMIC.opmode & OP_TXRXPEND) {
    ...
    dtostrf(buttonPushCounter, 1, 2, buffer);
    String res = buffer;
    res.getBytes(buffer, res.length() + 1);
    Serial.print("Count: " ); Serial.println(buttonPushCounter);
...
void loop()
{
    ...
    else
    {
     ...
     /* for (int i=0;i<sleepcycles;i++)
      {
          LowPower.powerDown(SLEEP_8S/*sleepforever, ADC_OFF, BOD_OFF);    //sleep 8 seconds
          /*interrupt        
      }
      */
